...
struct PlanNode
{
    ...
    {
        return std::unique_ptr<PlanNode>(new PlanNode{DbOperator::TableAccess, {tableName}, {}, {}});
    }
    ...
    static std::unique_ptr<PlanNode> CreateFilter(std::unique_ptr<PlanNode> &scan, ExprOper_ptr &condition)
    {
        return std::unique_ptr<PlanNode>(
            new PlanNode{DbOperator::Filter, std::vector<std::any>{}, std::move(scan), {}, std::move(condition)});
    }
